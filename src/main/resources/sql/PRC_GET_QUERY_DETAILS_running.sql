create or replace PROCEDURE PRC_GET_QUERY_DETAILS(P_CHAT_TEXT IN VARCHAR2,
  ...
  BEGIN
     ...
     /* Remove below words from the given input*/
      SELECT regexp_replace(P_CHAT_TEXT, 'team leads','team lead',1,0,'i')
      INTO input_string
      FROM dual;
          ...
          FOR t IN (SELECT DISTINCT SUBSTR (keyword1, 1, 11) Searchword,
            ...
            LOOP   
              ...
              EXECUTE IMMEDIATE    
                  ...
                  qry := 'select FIELDNAME,TABLENAME,WHERECOND'||' from ' || t.tablename ||' where upper('|| t.columnname ||') = upper(''' || keyword1
                       || ''')' ;
                         dbms_output.put_line( 'qry........' ||qry );
                   EXECUTE IMMEDIATE    
  ...
  /* Where clause column for date columns and number columns*/
     ...
     END IF;  
     dbms_output.put_line('1.....'||M_WHERE_CLAUSE);
     dbms_output.put_line('2.....'||dayval);
     dbms_output.put_line('3.....'||wherepart);
     dbms_output.put_line('4.....'||wherecond1);
